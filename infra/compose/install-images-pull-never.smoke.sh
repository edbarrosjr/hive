...
[[ "$offline_out" == *"HIVE is starting"* ]] || fail "--offline did not start"
...
[[ "$old_out" != *"HIVE setup failed:"* ]] || fail "old Compose --offline should not hard-fail: $old_out"
[[ "$old_out" == *"HIVE is starting"* ]] || fail "old Compose --offline should continue: $old_out"
