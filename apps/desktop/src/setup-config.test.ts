...
describe("server address normalization", () => {
  ...
  it("rejects cleartext link-local addresses used by cloud metadata endpoints", () => {
    ...
    // HTTPS to link-local still normalizes; the health probe must match HIVE.
  ...
  it.each(["", "   ", "not a url", "ftp://example.com", "file:///etc/passwd", "http://"])(
    "rejects an address that cannot reach a HIVE server (%s)",
    (value) => {
...
describe("HIVE health response", () => {
  it("requires the public RPC health contract", () => {
    expect(isHIVEHealth({ json: { ok: true, version: "0.1.0" } })).toBe(true);
    expect(isHIVEHealth({ json: { ok: true } })).toBe(false);
    expect(isHIVEHealth({ ok: true, version: "0.1.0" })).toBe(false);
  });
});
