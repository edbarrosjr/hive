...
import type { HIVEDesktopOAuthCallback } from "@rakazo/contracts";
...
export async function openBrowserAuth(
  ...
  options: {
    ...
    onCallback: (callback: HIVEDesktopOAuthCallback) => void;
    onClose?: () => void;
  },
): Promise<void> {
      ...
      for (const host of hosts) {
        ...
        const server = createServer({ maxHeaderSize: 8192 }, (request, response) => {
          ...
          if (
            ...
            response.writeHead(400).end("Sign-in callback rejected. Return to HIVE to retry.");
            return;
          }
          consumed = true;
          response.end("You can close this tab and return to HIVE.", close);
