 ...
 * fixtures omit the HIVE app-state marker.
 */
async function waitForMountedAppDocument(contents: Electron.WebContents) {
  ...
  while (Date.now() < deadline) {
    ...
    const ready = (await contents.executeJavaScript(`(() => {
      ...
      // Desktop e2e fixtures mount a plain page without HIVE app-state markers.
      if (appState === null) {
...
/** Setup IPC must only answer the setup window, never a connected HIVE server. */
  ...
  try {
    ...
    if (response.status >= 300 && response.status < 400) {
      return {
        ...
        url,
        error: "That address redirects elsewhere. Enter the final HIVE server address.",
      };
    }
    if (!response.ok) {
      return {
    ...
    if (!isHIVEHealth(health)) {
      return {
        ok: false,
        status: response.status,
        url,
        error: "That address did not respond like a HIVE server.",
      };
    }
    return {
      ok: true,
    ...
    try {
      ...
      if (setup.mode === "new") {
        ...
        if (managedUrl === null || !(await localStack.matchesDesiredStack())) {
          return {
            ok: false,
            error: "The app-managed HIVE services are not ready. Retry setup.",
          };
        }
