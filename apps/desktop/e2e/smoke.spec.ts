...
import type { HIVEDesktop } from "@rakazo/contracts";

const fixture = `<!doctype html>
<html lang="en">
  <head><meta charset="utf-8"><title>HIVE desktop smoke</title></head>
...
test("launches with a narrow preload bridge and an isolated renderer", async () => {
  ...
  try {
    ...
    await expect(page).toHaveTitle("HIVE desktop smoke");

    const renderer = await page.evaluate(async () => {
      const desktop = (window as typeof window & { rakazoDesktop?: HIVEDesktop }).rakazoDesktop;
