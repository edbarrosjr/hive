...
const APP_MARKER = "Existing HIVE instance ready";
...
test.beforeAll(async () => {
  server = createServer((request, response) => {
    ...
    response.end(
      `<!doctype html><html lang="en"><head><meta charset="utf-8"><title>HIVE</title></head><body><main>${APP_MARKER}</main></body></html>`,
    );
  });
...
test("first run asks whether to use a local or existing instance", async () => {
  ...
  await expect(setup.getByRole("heading", { name: "Welcome to HIVE" })).toBeVisible();
...
test("connecting to an existing instance verifies, saves, and opens it", async () => {
  ...
  await expect(setup.locator("#status")).toHaveText(`HIVE answered at ${serverUrl}.`);
...
test("a session-pending shell skeleton is not accepted as a ready app", async () => {
  const skeletonHtml = `<!doctype html><html lang="en"><head><meta charset="utf-8"><title>HIVE</title></head>
...
for (const { name, surface } of [
  ...
  test(`a post-session ${name} mount is accepted`, async () => {
    const readyHtml = `<!doctype html><html lang="en"><head><meta charset="utf-8"><title>HIVE</title>
</head>
...
test("a shell mount before workspace bootstrap is not accepted", async () => {
  const preBootstrapHtml = `<!doctype html><html lang="en"><head><meta charset="utf-8"><title>HIVE</title></head>
...
test("a session-ready marker without a route surface is not accepted", async () => {
  const emptyReadyHtml = `<!doctype html><html lang="en"><head><meta charset="utf-8"><title>HIVE</title></head>
...
test("a generic web page is not accepted as a HIVE server", async () => {
  const plain = createServer((_request, response) => {
    response.writeHead(200, { "content-type": "text/html; charset=utf-8" });
    response.end("<!doctype html><p>not HIVE</p>");
  });
  ...
  try {
    ...
    await expect(setup.locator("#status")).toHaveText(
      "That address did not respond like a HIVE server.",
    );
    await expect(async () => {
      await expect(readFile(path.join(userData, "setup.json"), "utf8")).rejects.toThrow();
    }).toPass();
