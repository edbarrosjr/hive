...
describe("agent content negotiation", () => {
  ...
  it("maps canonical and trailing-slash page paths to Markdown documents", () => {
    expect(getMarkdownDocument("/")).toContain("# HIVE");
    expect(getMarkdownDocument("/about/")).toContain("# About HIVE");
  ...
  it("publishes specific when-to-use instructions for agents", () => {
    ...
    expect(AGENT_INSTRUCTIONS).toContain("## When to use HIVE");
    expect(AGENT_INSTRUCTIONS).toContain("## How an agent should use HIVE");
  ...
  it("returns cache-safe Markdown responses and omits bodies for HEAD", async () => {
    const response = markdownResponse("# HIVE\n");
    ...
    await expect(response.text()).resolves.toBe("# HIVE\n");

    const headResponse = markdownResponse("# HIVE\n", "HEAD", 404);
