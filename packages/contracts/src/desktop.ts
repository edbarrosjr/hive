...
export interface HIVEDesktopUpdate {
...
export interface HIVEDesktopOAuthCallback {
  code: string;
  state?: string;
}

export interface HIVEDesktop {
  ...
  oauth: {
     ...
     */
    onCallback: (listener: (callback: HIVEDesktopOAuthCallback) => void) => () => void;
  };
}

/**
 * How the desktop app was pointed at a HIVE server during first-run setup.
