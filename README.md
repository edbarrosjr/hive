...
![HIVE — AI teammates you actually own](./docs/readme-hero.png)

HIVE is an open-source platform for running persistent AI teammates. It is available on the web,
...
HIVE is in beta. Learn more at [rakazo.com](https://rakazo.com).
...
The installer downloads the Compose files, creates `.env` with random secrets, and starts HIVE.
...
The Electron and Expo apps are clients of the same HIVE API used by the web app.
...
On first run the desktop app asks whether to run HIVE on this computer or connect to an existing
...
app verifies HIVE's health endpoint before saving, and later launches go straight to that
instance. The stack keeps running after the app quits; **Stop Local Stack** in the application
menu turns it off.

Use **Change HIVE Server…** in the application menu to reconnect. Closing that window without
...
Questions and ideas are welcome in the [HIVE Discord community](https://discord.gg/RWwKa2Sn7h).
